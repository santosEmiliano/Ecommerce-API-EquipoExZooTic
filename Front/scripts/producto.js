...
function cargarInformacionDOM(producto) {
  ...
  const descripcion = document.querySelector(".descripción") || document.querySelector(".descripcion"); 
  ...
  if (imagen) {
    ...
    imagen.onerror = () => { imagen.src = "media/logo.png"; };
  }
...
function activarBotonCompra(producto) {
    ...
    if (btnAgregar) {
        ...
        nuevoBtn.addEventListener("click", async () => {
            ...
            if (!userId) {
                Swal.fire({ title: "Identifícate", text: "Debes iniciar sesión.", icon: "info" });
                return;
            }
            ...
            if(cantidad > producto.existencias) {
                 Swal.fire({ title: "Ups", text: "No hay tantos animales disponibles.", icon: "warning" });
                 return;
            }
            ...
            try {
                ...
                if(typeof Toastify === 'function') {
                    Toastify({
                        ...
                        gravity: "bottom", 
                        position: "right", 
                        style: { background: "linear-gradient(to right, #7ab24e, #597a4a)" }
                    }).showToast();
                }
            } catch (error) {
